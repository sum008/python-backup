...
for i in range(0,n):
    ...
    ball_lis.append(b)
    y+=length
ball_lis[pinned[0][0]][pinned[0][1]].set_x(px1)
ball_lis[pinned[0][0]][pinned[0][1]].set_y(py)

ball_lis[pinned[1][0]][pinned[1][1]].set_x(px2)
ball_lis[pinned[1][0]][pinned[1][1]].set_y(py)
...
while run:
    ...
    p.draw.circle(display, (0,0,0), (int(ball_lis[0][0].get_x()),int(ball_lis[0][0].get_y())) , r, r)
    p.draw.circle(display, (0,0,0),(int(ball_lis[0][n-1].get_x()),int(ball_lis[0][n-1].get_y())) , r, r)
...
#     print(mouse_press[0])
    ...
    for i in range(0,len(ball_lis)):
        for j in range(0,len(ball_lis)):
            ...
            if i!=a and j!=b and (i,j) not in pinned:
                ...
                if randint(1,100)/10<=0.5:
                    
                    ball_lis[i][j].apply_wind()
                else:
                    ball_lis[i][j].remove_wind()
